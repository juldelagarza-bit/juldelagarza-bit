🇲🇽 Sobre mí
Soy analista financiero con background en diversos empredimientos y finanzas, en transición activa hacia el análisis de datos. Mi ventaja: combino el pensamiento crítico financiero y multiples experiencias en varios sectores con herramientas modernas de datos para generar insights que realmente mueven a los negocios aportando valor.
Actualmente completando el bootcamp de TripleTen Data Analytics, donde he desarrollado proyectos end-to-end con datos reales de industrias como telecomunicaciones, videojuegos, transporte urbanomy más por venir.
Lo que me diferencia: no solo analizo datos — entiendo el contexto de negocio detrás de cada número, lo que me permite comunicar hallazgos de forma clara a audiencias técnicas y no técnicas.

📍 Basado en México | Disponible para oportunidades remotas en USA, Europa idelamnete Suecia.
🎯 Buscando mi primer rol como Analista de Datos Jr.
🔭 Próximo proyecto: análisis de riesgos & fraudes inmobiliarios en Yucatán, México 
📊 Fan del storytelling con datos — los gráficos bellos y significativos.

🛠️ Stack Tecnológico
Lenguajes y herramientas de data & visualización
🐍 + <> Python | SQL | Power BI | Tableau | Databricks
![Uploading python-357ebd.svg…]()
<svg xmlns="http://www.w3.org/2000/svg" width="97.5" height="28" role="img" aria-label="PYTHON"><title>PYTHON</title><g shape-rendering="crispEdges"><rect width="97.5" height="28" fill="#357ebd"/></g><g fill="#fff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" text-rendering="geometricPrecision" font-size="100"><image x="9" y="7" width="14" height="14" href="data:image/svg+xml;base64,"/><text transform="scale(.1)" x="587.5" y="175" textLength="535" fill="#fff" font-weight="bold">PYTHON</text></g></svg>

🇺🇸 About Me
I'm a financial analyst with a background in various ventures and finance, actively transitioning to data analytics. My strength: I combine critical financial thinking and diverse experiences across multiple sectors with modern data tools to generate insights that truly drive business and add value.

Currently completing the TripleTen Data Analytics bootcamp, where I've developed end-to-end projects using real data from industries like telecommunications, video games, urban transportation, and more to come.

What sets me apart: I don't just analyze data—I understand the business context behind every number, allowing me to communicate findings clearly to both technical and non-technical audiences.

📍 Based in Mexico | Available for remote opportunities in the USA, Europe, and ideally Sweden.
🎯 Seeking my first role as a Junior Data Analyst.
🔭 Next project: Real estate risks analysis in Yucatán, Mexico.
📊 Fan of data storytelling—beautiful and meaningful charts.
